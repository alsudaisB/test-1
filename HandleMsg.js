...
const clph2 = require('caliph-api')
...
const genius = require('geniuslyricfinder')
const primbon = require('primbon-scraper')
const lolis = require('lolis.life')
const imgbase64 = require('fixed-image-to-base64')
const guitar = require('ultimate-guitar')
// d=C8B73274-F442-4F83-A752-C2AC0BEDCFF0; ig_nrcb=1; mid=Y1hG1wALAAGWplAK3kx8jJxus6X9; csrftoken=KL5lUffoxcBepaAs8LhK9kJr2Owe9nuV; sessionid=40194017622:V9xvzHWqOBPo6y:5:AYc3juBOjkIRKcivYxSiJ2xUWbczHoewC_WMxLt83g; ds_user_id=40194017622; shbid="11722\05440194017622\0541698265708:01f78bd22b57f8f3ed445b64daec848c50f6b1589debbca7bda4c3185f9c64c98c863412"; shbts="1666729708\05440194017622\0541698265708:01f76099b08f6841ee1c0eaf0060596c55d0cffb7eda7251c49f8b107a9b0a1a6154f4de"; datr=60ZYY8cuQe3zPQh_F54rjMmP; rur="PRN\05440194017622\0541698265770:01f733cd6a7da358958d996640742545f02409d6382e2ce97e931b442faf0f112c4bdc50"'
//=C8B73274-F442-4F83-A752-C2AC0BEDCFF0; ig_nrcb=1; mid=Y1hG1wALAAGWplAK3kx8jJxus6X9; csrftoken=KL5lUffoxcBepaAs8LhK9kJr2Owe9nuV; sessionid=40194017622:V9xvzHWqOBPo6y:5:AYc3juBOjkIRKcivYxSiJ2xUWbczHoewC_WMxLt83g; ds_user_id=40194017622; shbid="11722\05440194017622\0541698265708:01f78bd22b57f8f3ed445b64daec848c50f6b1589debbca7bda4c3185f9c64c98c863412"; shbts="1666729708\05440194017622\0541698265708:01f76099b08f6841ee1c0eaf0060596c55d0cffb7eda7251c49f8b107a9b0a1a6154f4de"; datr=60ZYY8cuQe3zPQh_F54rjMmP; rur="PRN\05440194017622\0541698265770:01f733cd6a7da358958d996640742545f02409d6382e2ce97e931b442faf0f112c4bdc50"'
...
const coktt = 'tt_webid_v2=6945801212629713817; tt_csrf_token=vzfBkKkr-EOrT20xEEo4U-tRk7GMoi3JYTCQ; msToken=cEQ0hj57Q7B9Cfx3MQyEQ8jiQixzqQalvU5t8ihn1wsa-V0cymuHBXmNPw0x67RrIEp8UVcyuT-0k_rdnkFr9syeyjpWd9GoXGl541wlp0SuXCtsEonyfj0=; _abck=BFA4ED2BCF8113EB1892E4521203139F~-1~~-1~-1~-1; bm_sz=1AAA8F5F521EFC275E002B5E66CE893C~~4469552~4471096'
const stalkuy = new TTScraper(coktt)
...
module.exports = HandleMsg = async (urbae, message) => {
	try {
		const { type, id, fromMe, from, t, sender, buttons, selectedButtonId, isGroupMsg, chat, chatId, caption, isMedia, mimetype, quotedMsg, quotedMsgObj, author, mentionedJidList, isViewOnce } = message
		let { body } = message
		...
		if (liststicker.includes(chats))
			try {
				...
				await urbae.sendImageAsStickerAsReply(from, `data:image/jpeg;base64,${getstick.toString('base64')}`, id, StickerMetadata)
			} catch (err) {
				console.log(err)
				urbae.reply(from, err.message, id)
			}
		...
		const getAfkPosition = (userId, file) => {
			let pstion = false
		...
		if (isGroupMsg && isVirtexOn && isBotGroupAdmins && message.type == 'chat') {
			try {
				if (message.text.length >= 50000) {
					console.log('virtex')
					urbae.deleteMessage(groupId, message.id, false)
					urbae.removeParticipant(groupId, sender.id)
					console.log(`Berhasil mengeluarkan ${pushname} dengan jumlah text ${chats.length}`)
					urbae.sendText(from, `Berhasil mengeluarkan ${pushname} dengan jumlah text ${chats.length}`)
				}
			} catch (err) {
				console.log(err)
				urbae.reply(from, err.message, id)
		...
		if (isAutoStikerOn && isMedia && isImage) {
			...
			await urbae.sendImageAsStickerAsReply(from, imageBase64, id, StickerMetadata)
		...
		if (sender.id.startsWith('62') || sender.id.startsWith('60')) {
		} else {
			urbae.contactBlock(sender.id)
		}


		urbae.setPresence(true)
		/*	
		if (isCmd && !isGroupMsg && !isOwnerB && !isPrem) {
			return urbae.reply(from, 'Bot hanya bisa digunakan didalam grup', id)
		}*/

		//urbae.sendSeen(chatId, false)
		function onlyGroup() {
			if (isGroupMsg == true) {
				return false
			} else {
		...
		if (isCmd && isMuted(chatId) && banChat() && !isBlocked && !isBanned || isOwnerB || isPrem) {
			switch (command) {
				...
				case prefix + 'eval':
				case prefix + 'ev':
					if (!isOwner && !isPrem) return await urbae.reply(from, 'Perintah ini hanya bisa digunakan oleh Owner!', id)
				...
				case prefix + 'fasttype':
					...
					urbae.sendText(from, `the word ${word}`, id).then(() => {
						urbae.awaitMessages(message, filter, {
							...
							.catch(collected => {
								urbae.sendText(from, 'Terlalu Lambat', id);
							});
					})
					break
				case prefix + 'p':
				...
				case prefix + 'menu':
					...
					const pictrand = 'https://telegra.ph/file/9bf33892d51e063b46890.jpg'
					urbae.sendAdvancedButtons(from, pictrand, [{ id: `${prefix}grupbot`, text: 'Link Group' }, { id: `${prefix}speed`, text: 'Speed' }], `Shinzu Bot`, `${menuId.help(prefix, jame, betime, prem, blockNumber, banned, hit, cts, waver)}`, '')
				...
				case prefix + 'leaderboard':
					...
					try {
						for (let i = 0; i < 10; i++) {
							...
							if (resp[i].level > 1000) {
								roles = 'God'
							}
							const returnpushname = await urbae.getContact(_level[i].id)
							leaderboard += `${i + 1}. ${returnpushname.pushname}\n- *XP*: ${_level[i].xp}\n- *Level*: ${_level[i].level}\n- *Role*: ${roles}\n\n`
						}
				...
				case prefix + 'owner':
					await urbae.sendMultipleContacts(from, ownerNumber)
					break
				case prefix + 'maps':
				case prefix + 'map':
				...
				/*case prefix + 'loli':
					...
					loliwrap.getSFWLoli()
						...
						//urbae.sendImageAsStickerAsReply(from, res.url, id, StickerMetadata)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				getSFWImage('neko')
				.then(neko => {
					if (neko == undefined) return urbae.reply(message.from, 'Error', id)
					urbae.sendAdvancedButtons(message.sender.id, neko, [{ id: `${prefix}nekoo`, text: 'Get Again' }], `Here's your cat`, 'Shinzu Bot - Thoriq Azzikra')
				})
				break
				case prefix + 'neko':
					urbae.reply(from, mess.wait, id)
					getSFWImage('neko')
						.then(nekos => {
							...
							if (isGroupMsg) {
							urbae.sendAdvancedButtons(from, nekos, [{ id: `${prefix}nekoo`, text: 'Get Again' }], `Here's your cat`, 'Shinzu Bot - Thoriq Azzikra')
							.catch(err => {
								console.log(err)
								urbae.reply(from, err.message, id)
							})
							} else {
								urbae.sendAdvancedButtons(from, nekos, [{ id: `${prefix}neko`, text: 'Get Again' }], `Here's your cat`, 'Shinzu Bot - Thoriq Azzikra')
							//urbae.sendImageAsStickerAsReply(from, nekos, id, StickerMetadata)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'boobs':
					...
					getnekos.nsfw.boobs()
						.then(boobs => {
							...
							urbae.sendImageAsStickerAsReply(from, boobs.url, id, StickerMetadata)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'gifhentai':
					...
					getnekos.nsfw.randomHentaiGif()
						.then(hentai => {
							...
							urbae.sendImageAsStickerAsReply(from, hentai.url, id, StickerMetadata)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'waifuu':
					getSFWImage('waifu')
					.then(result => {
						if (result == undefined) return urbae.reply(from, 'Error', id)
						urbae.sendAdvancedButtons(message.sender.id, result, [{ id: `${prefix}waifuu`, text: 'Get Again' }], `Here's your waifu`, 'Shinzu Bot - Thoriq Azzikra')
						.catch(err => {
							console.log(err)
						urbae.reply(from, err.message, id)
						})
				...
				case prefix + 'waifu':
					...
					getSFWImage('waifu')
						.then(result => {
							...
							if (isGroupMsg) {
							urbae.sendAdvancedButtons(from, result, [{ id: `${prefix}waifuu`, text: 'Get Again' }], `Here's your waifu`, 'Shinzu Bot - Thoriq Azzikra')
							.catch(err => {
								console.log(err)
								urbae.reply(from, err.message, id)
							})
							} else {
								urbae.sendAdvancedButtons(from, result, [{ id: `${prefix}waifu`, text: 'Get Again' }], `Here's your waifu`, 'Shinzu Bot - Thoriq Azzikra')
								//urbae.sendImageAsStickerAsReply(from, result, id, StickerMetadata)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'resend':
					...
					if (quotedMsgObj) {
						...
						const _mimetype = encryptMedia.mimetype
						const mediaData = await decryptMedia(encryptMedia)
						await urbae.sendImageAsStickerAsReply(from, `data:${_mimetype};base64,${mediaData.toString('base64')}`, id, StickerMetadata)
				...
				case prefix + 'join':
					...
					if (isPrem || isOwnerB) {
						...
						urbae.reply(from, 'Chat owner buat joinin', id)
					}
					break
				...
				case prefix + 'botstat': {
					...
					const groupsIn = groups.filter(x => x.groupMetadata.participants.map(x => [botNumber, '6288804225115@c.us'].includes(x.id._serialized)).includes(true))
					...
					if (quotedMsg && quotedMsg.type == 'audio' || quotedMsg && quotedMsg.type == 'ptt' || quotedMsg && quotedMsg.type == 'video') {
						try {
							...
							apirizky.search.whatmusic(mediaData)
								.then(detmus => {
									if (detmus.status == 1001) return urbae.reply(from, detmus.message, id)
									if (detmus.status == 3016) return urbae.reply(from, detmus.message, id)
									console.log(detmus)
									urbae.reply(from, `*Judul:* ${detmus.title}\n*Durasi:* ${detmus.duration} seconds\n*Artis:* ${detmus.artists}\n*Album:* ${detmus.album}\n*Genre:* ${detmus.genre_music}\n*Release Date:* ${detmus.release_date}`, id)
										.catch(err => {
											console.log(err)
											urbae.reply(from, err.message, id)
										})
				...
				case prefix + 'curi':
					...
					break
				case prefix + 'poll':
					if (!isGroupMsg) return urbae.reply(from, 'Hanya bisa digunakan didalam grup', id)
					if (!isGroupAdmins) return urbae.reply(from, 'Fitur ini hanya bisa digunakan oleh admin grup', id)
					if (!q) return urbae.reply(from, `Contoh format;\n${prefix}poll text poll | poll 1 | poll 2\nContoh: ${prefix}poll thoriq ganteng | ganteng bgt | ganteng sekali`, id)
					const polltxt = q.split('|')[0]
					const txtoption1 = q.split('|')[1]
					const txtoption2 = q.split('|')[2]
					if (!polltxt) return urbae.reply(from, 'Format pesan salah', id)
					if (!txtoption1) return urbae.reply(from, 'Format pesan salah', id)
					if (!txtoption2) return urbae.reply(from, 'Formst pesan slh', id)
					await urbae.B(from, {
						"pollCreationMessage": {
							"name": polltxt,
							"options": [
								{
									"optionName": txtoption1
								},
								{
									"optionName": txtoption2
								}
							],
				...
				case prefix + 'takestick':
					if (quotedMsg && quotedMsg.type == 'sticker' || quotedMsg && quotedMsg.type == 'image') {
						...
						await urbae.sendImageAsStickerAsReply(from, imageBase64, id, { author: `${clones}`, pack: `${packnames}` })
							.catch(err => {
								console.log(err)
								urbae.reply(from, 'Error!', id)
							})
						...
						const mediaData = await decryptMedia(message, uaOverride)
						const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
						await urbae.sendImageAsStickerAsReply(from, imageBase64, id, { author: `${authors}`, pack: `${packnames}` })
					} else {
				...
				case prefix + 'sgiffull':
					if (isMedia && type === 'video' || mimetype == 'sticker/gif') {
						...
						try {
							...
							await urbae.sendMp4AsSticker(from, vidbase, gifxyz, StickerMetadata, id)
								.then(async () => {
						...
						try {
							...
							await urbae.sendMp4AsSticker(from, videoBase64, gifxyz, StickerMetadata, id)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
				...
				case prefix + 'startgif': //By: Thoriq Azzikra
					if (isMedia && type === 'video' || mimetype === 'sticker/gif') {
						try {
							...
							await urbae.sendMp4AsSticker(from, vidBase64, { crop: true, loop: 0, fps: 30, square: 240, startTime: `00:00:${mulai}.0`, endTime: `00:00:${akhir}.0` }, StickerMetadatacrop, id)
						} catch (err) {
							console.log(err)
						...
						try {
							...
							const mediaData = await decryptMedia(quotedMsg, uaOverride)
							const videoBase64 = `data:${quotedMsg.mimetype};base64,${mediaData.toString('base64')}`
							await urbae.sendMp4AsSticker(from, videoBase64, { crop: true, loop: 0, square: 240, fps: 30, startTime: `00:00:${mulaini}.0`, endTime: `00:00:${akhirni}` }, StickerMetadatacrop, id)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						} catch (err) {
				...
				case prefix + 'sgifwm':
					if (isMedia && type === 'video' || mimetype === 'sticker/gif') {
						...
						try {
							...
							await urbae.sendMp4AsSticker(from, videoBase64, { crop: true, fps: 30, square: 240, startTime: `00:00:00.0`, endTime: `00:00:10.0`, loop: 0 }, { author: `${authorPacksgif}`, pack: `${namaPacksgif}`, keepScale: false }, id)
								.then(async () => {
									console.log(color(`Sticker Gif processed for ${processTime(t, moment())} seconds`, 'aqua'))
				...
				case prefix + 's':
					if (isMedia && type === 'image' || isMedia && type == 'video') {
						...
						if (message.type == 'video') {
							urbae.sendMp4AsSticker(from, imageBase64, gifcrop, StickerMetadata, id)
								.then(async () => {
									console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
								.catch(err => {
							...
							urbae.sendImageAsStickerAsReply(from, imageBase64, id, StickerMetadatacrop)
								.then(async () => {
									console.log(color(`Sticker processed for ${processTime(t, moment())} seconds`, 'aqua'))
								})
						}
				...
				case prefix + 'infogempa':
					const gmpa = require('gempa-id-info')
					gmpa.latestGempa()
						.then(res => {
							urbae.sendFileFromUrl(from, res.Shakemap.url, '', `- Tanggal: ${res.Tanggal}\n- Jam: ${res.Jam}\n- Koordinat: ${res.Coordinates}\n- Lintang: ${res.Lintang}\n- Bujur: ${res.Bujur}\n- Magnitudo: ${res.Magnitude}\n- Kedalaman: ${res.Kedalaman}\n- Wilayah: ${res.Wilayah}\n- Potensi: ${res.Potensi}\n- Dirasakan: ${res.Dirasakan}`, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'meme':
					if ((isMedia || isQuotedImage) && args.length >= 2) {
						...
						urbae.sendImageAsStickerAsReply(from, ImageBase64, StickerMetadata)
							.then(() => {
				...
				case prefix + 'ytmp3':
					...
					urbae.reply(from, mess.wait, id)
					if (args[0] == '-f') {
						y2mate.y2mateA(body.slice(11))
							.then(async (res) => {
								if (Number(res[0].size.split(' MB')[0]) >= 15) return urbae.reply(from, `Size is too big\nPlease download manually with the link below\n\nLink: ${res[0].link}`, id)
								await urbae.sendFileFromUrl(from, res[0].link.replace('https', 'http'), `${res[0].judul}`, '', id, false, false, false, true)
									.catch(err => {
										console.log(err)
										urbae.reply(from, 'Error, silahkan ulsngi', id)
									})
							})
					} else {
						y2mate.y2mateA(q)
							.then(async (res) => {
								if (Number(res[0].size.split(' MB')[0]) >= 15) return urbae.reply(from, `Size is too big\nPlease download manually with the link below\n\nLink: ${res[0].link}`, id)
								await urbae.sendAdvancedButtons(from, res[0].thumb, [{ id: `${prefix}lirik ${res[0].judul}`, text: 'Get Lyric' }], 'Press the button if you want get the lyric', 'Shinzu Bot - Thoriq Azzikra')
								await urbae.sendFileFromUrl(from, res[0].link.replace('https', 'http'), '', '', id)
									.catch(err => {
										console.log(err)
										urbae.reply(from, err.message, id)
									})
				...
				case prefix + 'emot':
					...
					emoji.get(q)
						.then(emoji => {
							...
							urbae.sendImageAsStickerAsReply(from, emoji.images[0].url, id, StickerMetadata)
								.then(() => {
				...
				case prefix + 'nhentaipdf':
					if (!isOwnerB) return urbae.reply(from, 'cuman owner coy', id)
					if (!q) return urbae.reply(from, 'kode nuklirnya mn', id)
					await urbae.reply(from, mess.wait, id)
					axios.get(`https://itztobz.me/api/nhentai-pdf?code=${q}`)
						.then(async (hentai) => {
							if (hentai.data.status == 400) return urbae.reply(from, hentai.data.message, id)
							//await urbae.sendFileFromUrl(from, hentai.data.result.thumbnail, '', `Title: {\nJapan: ${hentai.data.result.title.japanese}\n\nEnglish: ${hentai.data.result.title.english}\n}\nId: ${hentai.data.result.id}`, id)
							await urbae.sendFileFromUrl(from, hentai.data.result.download, `${hentai.data.result.title.japanese}`, '', id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'stickernobg':
					if (isMedia || isImage || isQuotedImage || isQuotedSticker) {
						...
						const mimetipeee = isQuotedImage || isQuotedSticker ? quotedMsg.mimetype : mimetype
						const mediaData = await decryptMedia(encryptMedia, uaOverride)
						const heynobg = `data:${mimetipeee};base64,${mediaData.toString('base64')}`
						urbae.sendImageAsStickerAsReply(from, heynobg, id, { author: authorstc, pack: packstc, removebg: true })
							.catch((err) => {
								console.log(err)
								urbae.reply(from, err.message, id)
							})
						...
						urbae.sendImageAsStickerAsReply(from, heynobg3, { author: authorstc, pack: packstc, removebg: true })
							.catch(err => {
								console.log(err)
								urbae.reply(from, err.message, id)
							})
				...
				case prefix + 'textmaker':
					if (isMedia && isImage || isQuotedImage) {
						...
						urbae.sendImageAsStickerAsReply(from, `https://api.memegen.link/images/custom/_/${textbot}.png?background=${imgs}`, StickerMetadata)
					} else {
				...
				case prefix + 'pinterestdown':
					...
					break
				case prefix + 'pindl':
					if (!q) return urbae.reply(from, 'Linknya mana?', id)
					if (!q.startsWith('https')) return urbae.reply(from, 'Itu bukan link blok', id)
					urbae.reply(from, mess.wait, id)
					clph2.downloader.pindl(q)
						.then(async (a) => {
							await urbae.sendAdvancedButtons(from, a.result, [{ id: `${prefix}donate`, text: 'Donate' }], `Buy me a coffee👇`, 'Shinzu Bot - Thoriq Azzikra', '')
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'pinterest':
					...
					const pinq1 = q.split('|')[0]
					const pinq2 = q.split('|')[1]
					//if (!pinq2) return urbae.reply(from,'Input salah', id)
					urbae.reply(from, mess.wait, id)
					hxzapi.pinterest(pinq1)
						.then(res => {
							if (pinq2 == undefined || pinq2 == '') {
								var beone = 1
							} else {
								var beone = pinq2
							}
							for (let i = 0; i < beone; i++) {
								...
								const rndompin = Math.floor(Math.random() * res.length)
								const hj = res[rndompin]
								urbae.sendFileFromUrl(from, hj, '', '', id)
									.catch(err => {
										urbae.reply(from, err.message, id)
									})
							}
				...
				case prefix + 'igreel':
					...
					if (!q.includes('instagram.com/reel')) return urbae.reply(from, 'Itu bukan link reels', id)
					await urbae.reply(from, mess.wait, id)
					getVidLink({ link: q })
						.then(async (res) => {
							const ftch = await fetch(res.API_Response.link)
							const ftchbuffer = await ftch.buffer()
							fs.writeFileSync('./media/reels.mp4', ftchbuffer)
							await urbae.sendFile(from, './media/reels.mp4', '', '', id)
							setTimeout(() => {
								fs.unlinkSync('./media/reels.mp4')
							}, 5000)
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'get':
					urbae.reply(from, mess.wait, id)
					await getSessionId('ifvckedup_', 'Buatbot')
						.then(async (res) => {
							console.log(res)
				...
				case prefix + 'instagram':
					...
					urbae.reply(from, mess.wait, id)
					const igeh = require('instagram-url-direct')
						instaV2.scrapeIg.fetchPost(args[0])
							.then(async (result) => {
								if (result.Error == true) {
									await urbae.reply(from, 'Error, bot akan mencoba dengan module lain', id)
						const igj = require('instagram-url-direct')
						igj(args[0])
							.then(async (res) => {
								...
								for (let i = 0; i < beone; i++) {
									await urbae.sendFileFromUrl(from, res.url_list[i], '', '', id)
								}
							})
								} else {
								if (args[1] == '' || args[1] == undefined) {
				...
				case prefix + 'xnxx':
					if (!isNsfwOn) return urbae.reply(from, mess.nsfwnoton, id)
					if (!isBokepers) return urbae.reply(from, mess.prem, id)
					if (!q) return urbae.reply(from, 'linknya mana', id)
					await urbae.reply(from, mess.wait, id)
					intansange.xnxxdl(q)
						.then(async (res) => {
							if (res.result.files.high == undefined || res.result.files.high == '' || res.result.files.high == null) {
								await urbae.sendFileFromUrl(from, res.result.files.low, '', '', id)
							} else {
								await urbae.sendFileFromUrl(from, res.result.files.high, '', '', id)
							}
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'xnxxsearch':
					...
					urbae.reply(from, mess.wait, id)
					intansange.xnxxsearch(q)
						.then(async (res) => {
							let tkj = 'Bokep\n\n'
							for (let i = 0; i < res.result.length; i++) {
								tkj += `Judul: ${res.result[i].title}\nInfo: ${res.result[i].info}\nLink: ${res.result[i].link}\n\n`
							}
							await urbae.reply(from, tkj, id)
						})
						.catch(err => {
							console.log(err)
							urbae.reply(from, err.message, id)
				...
				case prefix + 'uploadfile':
					if (isMedia || isQuotedVideo || isQuotedAudio || isQuotedImage) {
						await urbae.reply(from, mess.wait, id)
						const encrypu = isQuotedVideo || isQuotedAudio || isQuotedImage ? quotedMsg : message
						const myt = isQuotedVideo || isQuotedAudio || isQuotedFile || isQuotedImage ? quotedMsg.mimetype : mimetype
						const mediaData = await decryptMedia(encrypu, uaOverride)
						const upfile = await clph2.tools.uploadFile(mediaData)
						await urbae.reply(from, `Success Uploading File\n${upfile.result.url_file}`, id)
					} else {
						return urbae.reply(from, 'Reply atau post video/foto/file', id)
					}
					break
				case prefix + 'hidetag':
					if (!isGroupMsg) return urbae.reply(from, 'Fitur ini hanya bisa digunakan didalam grup', id)
					...
					if (isMedia || isQuotedImage || isQuotedVideo) {
						const enkrip = isQuotedImage || isQuotedVideo ? quotedMsg : message
						const thistype = isQuotedImage || isQuotedVideo ? quotedMsg.mimetype : mimetype
						const mediaData = await decryptMedia(enkrip, uaOverride)
						const stringuri = `data:${thistype};base64, ${mediaData.toString('base64')}`
						let whattext = q
						const getIdMembers = await urbae.getGroupMembersId(groupId)
						for (let i = 0; i < getIdMembers.length; i++) {
							whattext += `@${getIdMembers[i].replace('@c.us', '')}`
						}
						await urbae.sendFile(from, stringuri, '', whattext, id, false, false, false, true)
					} else {
				...
				case prefix + 'stalktt':
					...
					axios.get(`https://itztobz.me/api/tiktokstalk?username=${q}`)
						.then(User => {
							console.log(User)
							urbae.sendAdvancedButtons(from, User.data.result.avatar, [{ id: `${prefix}donate`, text: 'Buy Me A Coffee' }], `*- Username:* ${User.data.result.username}\n*- Nickname:* ${User.data.result.nickname}\n*- Followers:* ${User.data.result.followers} Followers\n*- Following:* ${User.data.result.followings}\n*- Verified:* ${User.data.result.verified}\n*- Private:* ${User.data.result.private}\n*- Total Likes:* ${User.data.result.hearts}\n*- Total Videos:* ${User.data.result.videos}\n*- Bio:* ${User.data.result.signature}`, 'Shinzu Bot - Thoriq Azzikra')
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
					...
					lyricsq.search(q)
						.then(res => {
							...
							urbae.reply(from, res, id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, `Maaf, lirik ${q} tidak dapat ditemukan`, id)
								})
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'chords':
					...
					guitar.firstData(q)
						.then(async (res) => {
							...
							const textchord = `Title: ${res.title} - ${res.artist}\nKey: ${res.key}\nType: ${res.type}\nChords:\n\n${res.chords}`
				...
				case prefix + 'facebook':
					...
					lol.downloader.facebook(q)
						.then(data => {
							console.log(data)
							//if (data.hd == '' || data.hd == undefined || data.hd == null) {
							urbae.sendFileFromUrl(from, data.sd, '', 'SD Quality', id)
								//} else {
								//urbae.sendFileFromUrl(from, data.hd, '', 'HD Quality', id)
								.catch(err => {
									console.log(err)
									urbae.reply(from, err.message, id)
								})
				...
				case prefix + 'twitter':
					...
					urbae.reply(from, mess.wait, id)
					lol.downloader.twitter(q)
						.then(async (res) => {
							if (res.quality_720 == '' || res.quality_720 == undefined) {
								urbae.sendFileFromUrl(from, res.quality_360, '', '360p', id)
							} else if (res.quality_360 == '' || res.quality_360 == undefined) {
								urbae.sendFileFromUrl(from, res.quality_270, '', '270p', id)
							} else {
								urbae.sendFileFromUrl(from, res.quality_720, '', '720p', id)
							}
						})
						.catch(err => {
							console.log(err)
				...
				case prefix + 'tiktok':
					...
					lol.downloader.tiktok(q)
						.then(async (a) => {
							console.log(a)
							await urbae.sendAdvancedButtons(from, a.media[1].url, [{ id: `${prefix}tiktokaudio ${q}`, text: 'Get Audio' }], `Caption: ${a.title}`, 'Shinzu Bot - Thoriq Azzikra', '')
								.catch(err => {
									urbae.reply(from, err.message, id)
								})
						})
				...
				case prefix + 'ytmp4':
					...
					y2mate.y2mateV(q)
						.then(async (res) => {
							if (res[0].link == undefined) {
								console.log('Error\n Trying to with the new link')
								scrape.youtubedown(q)
									.then(async (yt) => {
										if (Number(yt.size.split(' MB')[0]) >= 50) return urbae.reply(from, `Size is too big\nPlease download manually with the link below\nLink: ${yt.link}`, id)
										const downthu = await urbae.download(yt.link)
										await urbae.sendAdvancedButtons(from, downthu, [{ id: `${prefix}lirik ${yt.title}`, text: 'Get Lyric' }], `Hi @${sender.id.split('@c.us')[0]}, this is what you want?`, `Title: ${yt.title}\nSize: ${yt.size}\nQuality: 720p\n\nShinzu Bot - Thoriq Azzikra`, '')
									})
							} else {
								if (Number(res[0].size.split(' MB')[0]) >= 50) return urbae.reply(from, `Size is too big\nPlease download manually with the link below\n\nLink: ${res[0].link}`, id)
								await urbae.sendAdvancedButtons(from, res[0].link.replace('https', 'http'), [{ id: `${prefix}lirik ${res[0].judul}`, text: 'Get Lyric' }], `Hi @${sender.id.split('@c.us')[0]}, this is what you want?`, `Title: ${res[0].judul}\nSize: ${res[0].size}\nQuality: 720p\n\nShinzu Bot - Thoriq Azzikra`)
									.catch(err => {
										console.log(err)
										urbae.reply(from, err.message, id)
									})
				...
				case prefix + 'instastory':
					...
					instaV2.scrapeIg.fetchStories(xas1)
						.then(async (s) => {
							console.log(res)
							if (xas2 == '' || xas2 == null || xas2 == undefined) {
								var beome = 1
							} else {
								var beome = xas2
							}
							for (let i = 0; i < beome; i++) {
				...
				case prefix + 'spotify':
					...
					spos2(q)
						.then(async (res) => {
							...
							await urbae.sendAdvancedButtons(from, res.thumb, [{ id: `${prefix}lirik ${q}`, text: 'Get Lyric' }], `Hi @${sender.id.split('@c.us')[0]}, is this the song you want?`, `*• Title:* ${res.title}\n*• Artists:* ${hasil2}\n*• Release Date:* ${res.releaseDate}\n*• Popularity:* ${res.popularity}\n*• Url:* ${res.url}\n\n${mess.sendfileaudio}\n\nShinzu Bot - Thoriq Azzikra`, '')
				...
				case prefix + 'play': //lasilahkan kalian custom sendiri jika ada yang ingin diubah
					...
					yt.ytSearch(q)
						.then(async (res) => {
							...
							await urbae.sendAdvancedButtons(from, thumbnailytSD, [{ id: `${prefix}ytmp3 ${res[0].url}`, text: 'Get Audio' }, { id: `${prefix}ytmp4 ${res[0].url}`, text: 'Get Video' }, { id: `${prefix}ytmp3 -f ${res[0].url}`, text: 'Document' }], `Hi @${sender.id.split('@c.us')[0]} this is what you want?`, `Title: ${res[0].title}\nDuration: ${res[0].timestamp} seconds\nViews: ${res[0].views}\nUploaded: ${res[0].ago}\nChannel: ${res[0].author.name}\nUrl: ${res[0].url}\n\nShinzu Bot - Thoriq Azzikra`, '')
						})
						.catch(err => {
							console.log(err)
							urbae.reply(from, err.message, id)
				...
				case prefix + 'whatanime':
					if (isMedia && type === 'image' || isMedia && type == 'video' || quotedMsg.type == 'video' || quotedMsg && quotedMsg.type === 'image') {
						if (isMedia) {
							var mediaData = await decryptMedia(message, uaOverride)
						} else {
							var mediaData = await decryptMedia(quotedMsg, uaOverride)
						}
						const upcalip = await clph2.tools.uploadFile(mediaData)
						lol.search.whatanime(upcalip.result.url_file)
							.then(async (a) => {
								await ffmpeg(a.result[0].video)
									.format('mp4')
									.on('start', (s) => console.log(s))
									.on('progress', (p) => console.log(p))
									.on('error', (e) => {
										console.log(e)
										urbae.reply(from, e.mesage, id)
									})
									.on('end', async () => {
										let tekz = 'What Anime\n\n'
										for (let i = 0; i < a.result.length; i++) {
											tekz += `- Filename: ${a.result[i].filename}\n- Episode: ${a.result[i].episode}\n- Similarity: ${a.result[i].similarity}\n- From: ${a.result[i].from}\n- To: ${a.result[i].to}\n\n`
										}
						...
						urbae.reply(from, 'Reply foto atau video', id)
					}
					break
				...
				case prefix + 'delete':
					if (!isGroupMsg && quotedMsg) {
						...
						if (!isBotGroupAdmins) return urbae.reply(from, 'Jadikan Bot Admin terlebih dahulu', id)
						urbae.deleteMessage(from, quotedMsg.id, false)
					} else {
...
*Text : ${swrt}*`
				...
				case prefix + 'attp':
					...
					xteamapi.API.free.attp(txtx)
						.then(res => {
							...
							urbae.sendImageAsStickerAsReply(from, res.result, id, StickerMetadata)
								.then(async () => {
					...
					if (isMedia && isImage || isQuotedImage) {
						...
						canvas.Canvas.trigger(pepa)
							.then(async (buffer) => {
								...
								await urbae.sendImageAsStickerAsReply(from, `${sender.id}_trigger.jpg`, id, StickerMetadata)
				...
				case prefix + 'tahta':
					...
					urbae.sendImageAsStickerAsReply(from, tahtuy, id, StickerMetadata)
						.catch(() => {
							urbae.reply(from, 'Error', id)
						})
						.catch(err => {
...
_Desc di update oleh : @${chat.groupMetadata.descOwner.replace('@c.us', '')} pada *${moment(chat.groupMetadata.descTime * 1000).format('dddd, DD MMMM YYYY')}* pukul ${moment(chat.groupMetadata.descTime * 1000).format('HH:mm:ss')}_`)
				...
				case prefix + 'grupbot':
				case prefix + 'gcbot':
					const ch = `https://chat.whatsapp.com/IKLXRSa2o7x3dC3etWt2Q6`
					await urbae.sendLinkWithAutoPreview(from, ch, 'Skuyy joinn😁\n\nhttps://chat.whatsapp.com/IKLXRSa2o7x3dC3etWt2Q6')
					break
				...
				case prefix + 'tiktokmp3':
					...
					lol.downloader.tiktok(q)
						.then(res => {
							console.log(res)
							urbae.sendFileFromUrl(from, res.media[2].url, '', '', id)
						})
						.catch((err) => {
							console.log(err)
							urbae.reply(from, err.message, id)
