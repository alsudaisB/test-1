...
const y2mateV = async (yutub) => {
	...
	//let quality = $("div").find("#mp4 > table > tbody > tr:nth-child(1) > td:nth-child(3) > a").attr("data-fquality");
	let tipe = $("div").find("#mp4 > table > tbody > tr:nth-child(3) > td:nth-child(3) > a").attr("data-ftype");
	let output = `${judul}.` + tipe;
	let size = $("div").find("#mp4 > table > tbody > tr:nth-child(1) > td:nth-child(2)").text();
	...
	hasil.push({ thumb, judul, tipe, size, output, link });
	return hasil;
};
...
const y2mateA = async (yutub) => {
	function post(url, formdata) {
		...
		const supp = cheerio.load(meme.result);
		let link = supp("div").find("a").attr("href");
		hasil.push({ thumb, judul, size, tipe, output, link });
		return hasil;
	};
	
	module.exports = { y2mateA, y2mateV }
