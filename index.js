...
const start = async (urbae = new urbae()) => {
	...
	if (message.isGroupMsg == true && message.isNotification == true && message.subtype == 'description') {
	...
	} else if (message.isGroupMsg == true && message.isNotification == true && message.subtype == 'add' && !botnum.includes(message.recipients)) {
		if (message.recipients[0].startsWith('62') || message.recipients[0].startsWith('60')) {
		...
		*/
			...
			urbae.removeParticipant(message.from, message.recipients[0])
		}
	} else if (message.isGroupMsg == true && message.isNotification == true && message.subtype == 'remove' && !botnum.includes(message.recipients)) {
		/*const mediaData = await urbae.getProfilePicFromServer(message.recipients)
		if (mediaData == null || mediaData == undefined || mediaData == 'ERROR: 404' || mediaData == 'ERROR: 401') {
			var picture = errorpicture
		} else {
	...
	} else if (message.isGroupMsg == true && message.isNotification == true && message.subtype == 'invite' && !botnum.includes(message.recipients)) {
		if (message.recipients[0].startsWith('62') || message.recipients[0].startsWith('60')) {
		/*const mediaData = await urbae.getProfilePicFromServer(message.recipients)
		if (mediaData == null || mediaData == undefined || mediaData == 'ERROR: 404' || mediaData == 'ERROR: 401') {
                        var picture = errorpicture
		...
		//await urbae.sendFileFromUrl(message.from, picture, '', `Halo @${message.recipients[0].split('@')[0]}\nSelamat datang di grup *${message.chat.formattedTitle}*\n\nDeskripsi Grup: {\n\n${message.chat.groupMetadata.desc}\n\n}`)
	} else if (message.isGroupMsg == true && message.isNotification == true && message.subtype == 'leave' && !botnum.includes(message.recipients)) {
		/*const mediaData = await urbae.getProfilePicFromServer(message.recipients)
                if (mediaData == null || mediaData == undefined || mediaData == 'ERROR: 404' || mediaData == 'ERROR: 401') {
                        var picture = errorpicture
                } else {
	...
	urbae.onAddedToGroup(async (chat) => {
		for (let i = 0; i < chat.groupMetadata.participants.length; i++) {
			if (chat.groupMetadata.participants[i].id.startsWith('62') || chat.groupMetadata.participants[i].id.startsWith('60')) {
			} else {
				urbae.sendText(chat.id, 'Grup ini terdapat nomor +212, Bot akan keluar')
				await sleep(3000)
				urbae.leaveGroup(chat.groupMetadata.id)
			}
		}
	})
